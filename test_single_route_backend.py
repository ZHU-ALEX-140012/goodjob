...
TEST_USER_CONFIG = {
    ...
    'scoring': {
        ...
        'title_required_keywords': ['ai产品', 'ai应用'],
...
class SingleRouteBackendTests(unittest.TestCase):
    ...
    def test_count_mode_delivers_when_matched_enough_and_title_gate_blocks(self):
        from core import evaluateSingleRouteDelivery

        ok = evaluateSingleRouteDelivery(
            '# 职位名称\nAI产品工程师\n\n# 薪资范围\n20-30K\n\n# 职位描述\n负责 Claude Code、Codex、智能体、工作流与代码生成调试部署'
        )
        self.assertEqual(ok['score'], 100)
        self.assertGreaterEqual(ok['matched_count'], 4)

        blocked = evaluateSingleRouteDelivery(
            '# 职位名称\nJava后端工程师\n\n# 薪资范围\n20-30K\n\n# 职位描述\n负责 Claude Code、Codex、智能体、工作流与代码生成调试部署'
        )
        self.assertEqual(blocked['score'], 0)
        self.assertEqual(blocked['matched_field'], 'title_required_miss')
